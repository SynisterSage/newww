...
export class DatabaseStorage implements IStorage {
  ...
  async getCourseHoles(course?: string): Promise<CourseHole[]> {
    // MemStorage method - keep as is for now since we don't have course holes in DB yet
    return [];
  }
  ...
  async getRounds(userId: string): Promise<Round[]> {
    // MemStorage method - keep as is for now since we don't have rounds in DB yet
    return [];
  }
  ...
  async createRound(insertRound: InsertRound): Promise<Round> {
    // MemStorage method - keep as is for now since we don't have rounds in DB yet
    throw new Error("Rounds not implemented in database storage yet");
  }
