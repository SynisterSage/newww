...
export interface IStorage {
  ...
  getAllTeeTimesForAdmin(): Promise<TeeTime[]>;
  updateTeeTimeStatus(id: string, updates: Partial<TeeTime>): Promise<TeeTime | undefined>;
  getAllOrdersForAdmin(): Promise<Order[]>;
  updateOrderStatus(id: string, updates: Partial<Order>): Promise<Order | undefined>;
  getAllMembersForAdmin(): Promise<User[]>;
  updateMember(id: string, updates: Partial<User>): Promise<User | undefined>;
  getAllEventsForAdmin(): Promise<any[]>;
  updateEvent(id: string, updates: any): Promise<any>;
  updateEventRegistration(eventId: string, registrationId: string, updates: any): Promise<any>;
}
...
export class MemStorage implements IStorage {
  ...
  // Admin management methods
  async getAllTeeTimesForAdmin(): Promise<TeeTime[]> {
    return Array.from(this.teetimes.values()).map(teeTime => ({
      ...teeTime,
      bookedBy: teeTime.userId ? this.users.get(teeTime.userId)?.username : undefined,
      bookedEmail: teeTime.userId ? `${this.users.get(teeTime.userId)?.username}@packanackgolf.com` : undefined,
      players: 1,
  ...
  async updateTeeTimeStatus(id: string, updates: Partial<TeeTime>): Promise<TeeTime | undefined> {
    return this.updateTeetime(id, updates);
  }

  async getAllOrdersForAdmin(): Promise<Order[]> {
    return Array.from(this.orders.values()).map(order => ({
      ...order,
      memberName: this.users.get(order.userId!)?.username || "Unknown Member",
      memberEmail: `${this.users.get(order.userId!)?.username}@packanackgolf.com`,
  ...
  async updateOrderStatus(id: string, updates: Partial<Order>): Promise<Order | undefined> {
    return this.updateOrder(id, updates);
  }

  async getAllMembersForAdmin(): Promise<User[]> {
    return Array.from(this.users.values()).map(user => ({
      ...user,
      email: `${user.username}@packanackgolf.com`,
      joinDate: "2023-01-15",
  ...
  async updateMember(id: string, updates: Partial<User>): Promise<User | undefined> {
  ...
  async getAllEventsForAdmin(): Promise<any[]> {
    // Mock events data for admin interface
    return [
      {
        id: "event-1",
        ...
        status: "upcoming",
        entryFee: "75.00",
        prizes: "Trophy + $500 Pro Shop Credit",
        createdAt: new Date().toISOString(),
        registrations: [
          {
            id: "reg-1",
            memberName: "John Wellington",
            memberEmail: "john.wellington@packanackgolf.com",
          ...
          },
          {
            ...
            memberEmail: "sarah.mitchell@packanackgolf.com",
            registrationDate: new Date().toISOString(),
            status: "registered",
            notes: "Needs cart rental"
          }
        ]
      },
      {
        ...
        description: "Bring a guest for this special tournament",
        date: "2025-08-22",
        time: "10:00 AM", 
        location: "Championship Course",
        maxParticipants: 40,
  ...
  async updateEvent(id: string, updates: any): Promise<any> {
    ...
    const events = await this.getAllEventsForAdmin();
  ...
  async updateEventRegistration(eventId: string, registrationId: string, updates: any): Promise<any> {
    // Mock implementation for event registration updates
    const events = await this.getAllEventsForAdmin();
    const event = events.find(e => e.id === eventId);
    if (!event) return undefined;
    
    const registration = event.registrations.find((r: any) => r.id === registrationId);
